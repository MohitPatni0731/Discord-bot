...
for i in range(100):
  ...
  if current_price >= upper_limit:
    high_price_alert = "ALERT HIGH PRICE: Current price of 1 British Pound is = "+str(current_price)+" Indian Rupees"
    hook.send(high_price_alert)
    upper_limit += 0.50

  elif current_price <= lower_limit:
    low_price_alert = "ALERT LOW PRICE: Current price of 1 British Pound is = "+str(current_price)+" Indian Rupees"
    hook.send(low_price_alert)
    lower_limit -= 0.50

  else:
    current_price_alert = "Current price of 1 British Pound is = "+str(current_price)+" Indian Rupees"
